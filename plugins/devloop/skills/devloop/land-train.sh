...
if [ -z "$github_default" ]; then
  say "usage: could not read the default branch of ${SLUG} from 'gh repo view ${SLUG} --json defaultBranchRef', so nothing is known about whether ${BASE} is its default - nothing touched"
  exit 6
fi
