...
const AMBIGUOUS_COLUMNS = 1;
const NARROW_COLUMNS = 1;

export type EastAsianWidth = "wide" | "ambiguous" | "narrow";

function within(ranges: readonly number[], code: number): boolean {
  let low = 0;
  let high = ranges.length / 2 - 1;
  while (low <= high) {
    const middle = (low + high) >> 1;
    const start = ranges[middle * 2] ?? 0;
    const end = ranges[middle * 2 + 1] ?? 0;
    if (code < start) {
      high = middle - 1;
    } else if (code > end) {
      low = middle + 1;
...
export function eastAsianWidth(code: number): EastAsianWidth {
  if (within(WIDE_RANGES, code)) {
    return "wide";
  }
  return within(AMBIGUOUS_RANGES, code) ? "ambiguous" : "narrow";
}

export function characterColumns(character: string): number {
  const code = character.codePointAt(0);
  if (code === undefined) {
    return 0;
  }
  switch (eastAsianWidth(code)) {
    case "wide":
      return WIDE_COLUMNS;
    case "ambiguous":
...
export function displayWidth(text: string): number {
  ...
  for (const character of text) {
    columns += characterColumns(character);
  }
  return columns;
}
